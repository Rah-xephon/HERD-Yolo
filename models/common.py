...
class Ann_Conv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros', marker='b'):
        super(Ann_Conv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode)
        self.marker = marker
        # self.relu = nn.ReLU()
        
    def forward(self, input):
        # just set time_window as 1
        weight = self.weight# 
        ...
        # print(input.size())
        c1 = torch.zeros(1, input.size()[1], self.out_channels, h, w, device=input.device)
        # input[0] = self.relu(input[0])
        c1[0] = F.conv2d(input[0], weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        return c1

class Ann_Conv_1(nn.Module):
        # Standard convolution：基础的卷积加上normalize
    def __init__(self, c1, c2, k, s, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = Ann_Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
...
class Ann_BasicBlock_2(nn.Module):
    def __init__(self, in_channels, out_channels,k_size=3,stride=1):
        ...
        self.residual_function = nn.Sequential(
            nn.ReLU(in_channels),
            Ann_Conv2d(in_channels, out_channels, kernel_size=k_size, stride=stride, padding=pad, bias=False),
            batch_norm_2d(out_channels),
            nn.ReLU(out_channels),
            Ann_Conv2d(out_channels, out_channels, kernel_size=k_size, padding=pad, bias=False),
            batch_norm_2d(out_channels),
            )
        # 残差block的jump
        ...
        if stride != 1 or in_channels != out_channels:
            self.shortcut = nn.Sequential(
                ...
                nn.ReLU(in_channels),
                Ann_Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False),
                batch_norm_2d(out_channels),
            )
            
    def forward(self, x):
        # out_channels = input_channels时，返回input x 加上恒等变换的结果；
        #o ≠ i 时，shortcut加一层池化和卷积
        return (self.residual_function(x) + self.shortcut(x))

class Ann_Concat_res2(nn.Module):
    def __init__(self, in_channels, out_channels,k_size=3,stride=1,e=0.5):
        ...
        if in_channels<out_channels:
            self.shortcut = nn.Sequential(                 
                nn.ReLU(in_channels),      
                Ann_Conv2d(in_channels, out_channels-in_channels, kernel_size=1, stride=1, bias=False),
    ...
    def __init__(self, in_channels, growth_rate, num_layers):
        ...
        for i in range(self.num):
            self.DenseBlock.append(
                nn.Sequential(
                 nn.ReLU(),
                 Ann_Conv2d(in_channels+i*self.rate, self.rate,
                             kernel_size=3, stride=1, padding=1, bias=False),
                 batch_norm_2d(self.rate),
            ))
...
# 最大池化层
# 脉冲：（time_window *batch_size *feature *high *width）
...
# our pre Spiking dense block
class Spiking_DenseBlock(nn.Module):#
    def __init__(self, in_channels, growth_rate, num_layers, stride ):
        super().__init__()
        # c_ = int(out_channels * e)  # hidden channels  
        # if k_size==3:
        #     pad=1
        ...
        for i in range(self.num):
            self.DenseBlock.append(
                nn.Sequential(
                 ...
                 Snn_Conv2d(in_channels+i*self.rate, self.rate,
                             kernel_size=3, stride=1, padding=1, bias=False),
                 batch_norm_2d(self.rate),
            ))
            # print("dense_"+str(i)+"input="+str(in_channels+i*self.rate))

        # self.basic_dense_function = nn.Sequential(
        #     mem_update(act=False),
        ...
        #         batch_norm_2d(out_channels-in_channels),
        #     )
        self.pools=nn.MaxPool3d((1, self.stride, self.stride), 
    ...
    def forward(self, x):
        ...
        # print(len(self.DenseBlock))
        for layer in self.DenseBlock:
            out_i = out
            out_i = layer(out_i)
            # print("print out size :" + str(out.size()))
            # print("print temp size:" + str(temp.size()))
            out = torch.cat([out_i, out], dim=2)
            # print(out.size())
        out = self.pools(out)
        return out
...
# Spiking CSP_Dense Block
class CSP_DenseBlock(nn.Module):

    def __init__(self, in_channels, rate, num_layers, stride = 2, part_ratio=0.5):
        ...
        # print(self.part2_chnls)
        self.dense = Spiking_DenseBlock(self.part2_chnls, rate, num_layers)
    ...
    def forward(self, x):
        ...
        out = torch.cat((part1, part2), dim=2)
        # out = self.pools(out)
        return out
...
class Conv_1(nn.Module):
    # Standard convolution：基础的SNN卷积加上normalize
    def __init__(self, c1, c2, k, s, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = Snn_Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = batch_norm_2d(c2)
...
class Conv_2(nn.Module):
    ...
    def __init__(self, c1, c2, k, s, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = Snn_Conv2d(c1, c2, k, s, 
...
# 3D POOLS for spiking_data
class Pools3(nn.Module):
