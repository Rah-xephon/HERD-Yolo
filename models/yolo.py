...
class Model(nn.Module):
    ...
    #!! data_convert: this method convert normal data into five dimensions containing timewindow
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        # transport "spiking" into calculating processes
    ...
    def _forward_once(self, x, profile=False, visualize=False):#执行这里【1,256,16,16】
        y, dt = [], []  # outputs
        for m in self.model:
            if m.f != -1:  # if not from previous layer it will find x by index "m.f"
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    # the first c2 = ch[-1], which actually is input_channels
    layers, save, c2 = [], [], ch[-1]  # layers, savelist, ch out
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings
        for j, a in enumerate(args):
            try:
                # make sure all args aren't string
        ...
        if m in [Conv, MixConv2d, CrossConv,Ann_Concat_res2,
                 Conv_2,Conv_3,Ann_Conv_1,Ann_BasicBlock_2,AnnConv,
                 BasicBlock,BasicBlock_1,BasicBlock_2,Conv_A,
                 ConcatBlock_ms,BasicBlock_ms,Conv_1,Concat_res2,]:
            # those layers need both input_channels and output_channels 
            c1, c2 = ch[f], args[0]
            ...
            if m in []:
                ...
                n = 1
        # c2 = c1 when we use normalization layer
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
        elif m is Pools:
            c2 = ch[f]
        elif m in [DenseBlock,CSP_DenseBlock,Spiking_DenseBlock]:
            # calculate the channels of CSP block
            c2 =  ch[f] + args[1]*args[2]
        elif m is Concat:
